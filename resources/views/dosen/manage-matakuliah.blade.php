...
<div class="container mx-auto px-4 py-8 max-w-7xl" x-data="{ activeTab: 'absensi' }">
    <!-- Button kembali -->
        ...
        <a href="{{ route('dosen.matakuliah', ) }}" class="flex items-center text-blue-600 hover:text-blue-800 transition-colors">
        ...
        <ul class="flex flex-wrap -mb-px">
            <li>
                <button @click="activeTab = 'absensi'" 
                        :class="activeTab === 'absensi' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" 
            ...
            <li>
                <button @click="activeTab = 'mahasiswa'" 
                        :class="activeTab === 'mahasiswa' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" 
                        class="inline-block p-4 border-b-2 font-semibold text-sm">
                    Daftar Mahasiswa
                </button>
            </li>
    ...
    <div x-show="activeTab === 'absensi'" class="bg-white shadow-md rounded-lg">
        <div class="px-6 py-5 flex justify-between items-center">
            ...
            <form action="{{ route('dosen.generate.absensi', $matkul->id) }}" method="POST">
                @csrf
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white text-sm font-medium py-2 px-4 rounded-lg flex items-center gap-2 transition-colors">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button onclick="openEditModal('{{ $absensi->id }}', '{{ $absensi->topik }}', '{{ $absensi->tanggal }}')" class="text-blue-600 hover:text-blue-900 mr-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </button>
                            <button onclick="toggleAbsensi({{ $absensi->id }})" class="text-sm {{ $absensi->is_active ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600' }} text-white py-1 px-3 rounded">
                                {{ $absensi->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                            </button>
                            @if($absensi->is_active)
                            <a href="{{ route('dosen.absensi.barcode', $absensi->id) }}" class="ml-2 text-sm bg-purple-500 hover:bg-purple-600 text-white py-1 px-3 rounded">
                                Lihat Barcode
                            </a>
                            @endif
                        </td>
    ...
    <div x-show="activeTab === 'mahasiswa'" class="bg-white shadow-md rounded-lg">
        <div class="px-6 py-5">
            ...
            <p class="text-sm text-gray-500">Jumlah mahasiswa: {{ isset($mahasiswas) ? $mahasiswas->count() : 'Tidak terdefinisi' }}</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $mahasiswa->user->name ?? 'Nama tidak tersedia' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $mahasiswa->nim ?? 'NIM tidak tersedia' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div class="mt-3 text-center">
                ...
                <form id="editForm" action="" method="POST">
                    ...
                    <div class="mt-2 px-7 py-3">
                        <input type="text" id="edit_topik" name="topik" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        <input type="date" id="edit_tanggal" name="tanggal" required class="mt-4 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
